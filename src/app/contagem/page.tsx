...
export default async function CountPage() {
  ...
  return <main className="count-report"><div className="count-report__actions no-print"><Link href="/">← Voltar</Link><span className="muted">Use Ctrl/Cmd+P para imprimir</span></div><header className="count-report__header"><h1>MULTISHOW FLV — CONTAGEM PARA PEDIDO</h1><p>Loja: {storeName}</p><p>Data: {new Intl.DateTimeFormat("pt-BR").format(new Date())}</p><p>Responsável: ______________________________</p></header><table className="count-table"><thead><tr><th>Cód</th><th>Produto</th><th>Est.</th><th>Ped.</th><th>Cód</th><th>Produto</th><th>Est.</th><th>Ped.</th></tr></thead><tbody>{rows.map(([left, right], index) => <tr key={index}>{left ? <><td>{left.erpCode}</td><td>{left.name} ({left.unit})</td><td>______</td><td>______</td></> : <><td /><td /><td /><td /></>}{right ? <><td>{right.erpCode}</td><td>{right.name} ({right.unit})</td><td>______</td><td>______</td></> : <><td /><td /><td /><td /></>}</tr>)}</tbody></table></main>;
}
