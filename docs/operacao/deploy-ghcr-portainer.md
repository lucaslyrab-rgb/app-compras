...
O primeiro reteste (`94fe059`) também confirmou a publicação, mas encontrou uma corrida de concorrência na promoção do manifesto: dois workflows tentaram atualizar `main` simultaneamente. O terceiro commit foi executado somente após a fila estabilizar e comprovou o fluxo completo. A configuração de concorrência deve ser mantida/avaliada antes de liberar múltiplos deploys simultâneos.
