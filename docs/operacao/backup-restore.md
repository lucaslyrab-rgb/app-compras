...
Foi executado um ensaio isolado com `pg_dump --format=custom`, criptografia `age` e restauração em `app_compras_restore_test`. As contagens restauradas foram `3` lojas, `74` produtos e `21` exclusivos; o database temporário foi removido ao final. O ensaio efetivo S3/off-host e o rollback operacional da stack continuam pendentes até execução com as credenciais rotacionadas e definição da janela de mudança.
